    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.laporan.pdf', request()->query()) }}" target="_blank"
                class="px-5 py-2.5 bg-accent text-white rounded-full font-medium text-sm hover:bg-accent/90 transition shadow-md inline-block">
                Ekspor PDF
            </a>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md p-4">
            <form action="{{ route('admin.laporan') }}" method="GET" class="flex flex-wrap gap-3 items-end">
                    ...
                    <select name="child_id" class="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm bg-white">
                        ...
                        @foreach ($children as $child)
                            <option value="{{ $child->id }}" {{ request('child_id') == $child->id ? 'selected' : '' }}>
                                {{ $child->name }} ({{ $child->user->name }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="w-32">
                ...
                <button type="submit"
                    class="px-5 py-2 bg-primary text-white rounded-full text-sm font-medium">Filter</button>
            </form>
        </div>
                        ...
                        @forelse($records as $i => $r)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <span
                                        class="px-2.5 py-1 rounded-full text-xs font-medium {{ $s === 'Baik' ? 'bg-green-100 text-green-700' : ($s === 'Kurang' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700') }}">{{ $s }}</span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-8 text-center text-gray-500">Belum ada data.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
